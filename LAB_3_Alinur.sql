...
union distinct select * from classroom where building = 'Packard';

--C)Find all courses offered by the Computer Science department;
select * from course where dept_name = 'Comp. Sci.';

--D)Find all courses offered during fall;
select * from course where course_id = some( select course_id from takes where semester = 'Fall');
...
--2.
--A)For each department, find the average salary of instructors in that department and list them in ascending order. Assume that every department has at least one instructor;
...
union select * from instructor where dept_name = 'Philosophy'
union select * from instructor where dept_name = 'Music';

--F)Find all instructors who taught in the 2018 year but not in the 2017 year;
select * from instructor where ID = some(select ID from teaches where year = '2018' )
except select * from instructor where ID = some(select ID from teaches where year = '2017');


--3.
--A)Find all students who have taken Comp. Sci. course and got an excellent grade (i.e., A, or A-) and sort them alphabetically;
select * from student where dept_name = 'Comp. Sci.' and ID = some (select ID from takes where grade = 'A')
union  select * from student where dept_name = 'Comp. Sci.' and ID = some (select ID from takes where grade = 'A-') order by name;

--B)Find all advisors of students who got grades lower than B on any class;
select name from instructor where ID = some(select i_ID from advisor where s_ID = some( select ID from takes where grade != 'A' OR grade !='A-' OR grade !='B+' OR grade !='B'));

--C)Find all departments whose students have never gotten an F or C grade;
select distinct on(dept_name) dept_name from student where ID = some (select ID from takes where grade != 'F' or grade != 'C');

--E)Find all courses offered in the morning hours (i.e., courses ending before 13:00);
select * from course where course_id = some (select course_id from section where time_slot_id = some (select time_slot_id
    from time_slot  where end_hr < 13));
